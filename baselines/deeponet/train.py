"""Train DeepONet with project-root-relative paths."""
...
def parse_args():
    ...
    parser.add_argument("--checkpoint-dir", default=str(PROJECT_ROOT / "baselines/deeponet/checkpoints"))
...
def main():
    ...
    trunk_dim = sample_trunk.shape[-1]
    print(f"Branch dim: {branch_dim}, Trunk dim: {trunk_dim}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loader:
            ...
            if avg_val < best_val:
                best_val = avg_val
                torch.save(model.state_dict(), ckpt_dir / "deeponet_best.pt")
        else:
            avg_val = float("nan")

        if epoch % 10 == 1 or epoch == 1:
            print(f"Epoch {epoch}/{args.epochs}  Train {avg_train:.6e}  Val {avg_val:.6e}  LR {scheduler.get_last_lr()[0]:.2e}")

    train_time = time.time() - t0
    print(f"Training time: {train_time:.2f}s")

    torch.save(model.state_dict(), ckpt_dir / "deeponet_final.pt")
